#!/usr/bin/env python3
"""
Aura Agent — a small, monitorable harness for transforming names/phrases
into higher-"aura" equivalents.
...
Usage:
    ...
    python aura_agent.py "Kevin" --tone ironic
...
class AuraAgent:
    def __init__(self, client: "anthropic.Anthropic" = None, log_path: Path = LOG_PATH):
        self.client = client or anthropic.Anthropic()
        self.log_path = log_path

    def _call(self, system: str, user: str, max_tokens: int = 1000) -> dict:
    ...
    def self_critique(self, generation: dict) -> dict:
        ...
        return self._call(CRITIQUE_SYSTEM_PROMPT, user_msg, max_tokens=1200)
...
def main():
    parser = argparse.ArgumentParser(description="Aura Agent — name/phrase aura transformer")
    ...
    agent = AuraAgent(log_path=Path(args.log))
